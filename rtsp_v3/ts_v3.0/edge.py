...
# --------- IMPORTANT FIX ----------
# One clock for BOTH RGB(meta) and Depth(header) so dt is meaningful on server
def common_ts_ns() -> int:
...
def build_cal_packet(w: int, h: int) -> bytes:
    ...
    # timestamp here is informational (not used for pairing)
    ts_ns = common_ts_ns()
...
def main():
    ...
    # MUST count exactly frames written into ffmpeg stdin
    vid_frame_id = 0
    ...
    with dai.Pipeline() as pipeline:
        ...
        try:
            while pipeline.isRunning() and not quit_event.is_set():
                ...
                # Depth -> DPT0 with SAME clock base
                if now >= next_depth_t:
                    dmsg = q_depth.tryGet()
                    if dmsg is not None:
                        while True:
