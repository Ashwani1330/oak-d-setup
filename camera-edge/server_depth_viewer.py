#!/usr/bin/env python3
#server_depth_viewer.py: UDP depth viewer + point cloud (debug-friendly defaults).
...
def main():
    ...
    while True:
        ...
        if magic == MAGIC_CAL:
            ...
            K_depth = np.array(floats[9:18], dtype=np.float32).reshape(3, 3)
        ...
        # Open3D point cloud
        if args.o3d and HAS_O3D and (ok % max(1, args.o3d_every) == 0) and (K_depth is not None):
            pts = depth_to_points(depth_roi, K_depth, stride=max(1, args.pcd_stride),
